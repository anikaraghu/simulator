...
void worker_sendPacket(Packet* packet) {
    ...
    if(bootstrapping || chance <= reliability || packet_getPayloadLength(packet) == 0) {
        ...
        SimulationTime delay = (SimulationTime) ceil(latency * SIMTIME_ONE_MILLISECOND); // TODO ANIKA + GRACE 
       // SimulationTime delay = (SimulationTime) ceil((latency + 0.1) * SIMTIME_ONE_MILLISECOND); // TODO ANIKA + GRACE 
